...
#include <iostream>
#include <ros/ros.h>
inline Eigen::Vector3d dMultiply(const Eigen::Vector3d a, const Eigen::Vector3d b)
{
                                ...
                                Controller_Output_t &u)
{
  ...
  bool so3Control = true;
  if (so3Control)
  {
    Eigen::Vector3d Zw = Eigen::Vector3d(0, 0, 1);
    Eigen::Vector3d F_des = des.a + dMultiply(Kp, des.p - odom.p) + dMultiply(Kv, des.v - odom.v) + param_.mass * param_.gra * Zw;
    Eigen::Vector3d Zb = (odom.q * Zw).normalized();
    u.thrust = F_des.dot(Zb);
    // std::cout<< u.thrust << std::endl;

    Eigen::Vector3d Zb_des = F_des.normalized();
    double yaw = fromQuaternion2yaw(des.q);
    Eigen::Vector3d Xc_des(cos(yaw), sin(yaw), 0);
    Eigen::Vector3d Yb_des = Zb_des.cross(Xc_des).normalized();
    Eigen::Vector3d Xb_des = Yb_des.cross(Zb_des);

    Eigen::Matrix3d R;
    R << Xb_des, Yb_des, Zb_des;
    u.q = imu.q * odom.q.inverse() * Eigen::Quaterniond(R);
  }
  else
  {
